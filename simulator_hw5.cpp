...
void simulate(FILE* inputFile, FILE* outputFile, char* para)
{
  ...
  int reg_size = 50;	//architectural reg: remain 50 all the time
	...
	printf("mapTable:\n");
	for(i = 0; i < reg_size; i++) {
		printf("%d	", mapTable[i].phys_reg_source_index);
		printf("%d	", mapTable[i].arch_reg_source_index);
	}
	printf("\n");*/
	int PHY_REG_SIZE = 2048;
	printf("PHY_REG_SIZE: %d\n", PHY_REG_SIZE);
	int new_reg;
	...
	/*
	printf("Register Board: \n");
	for (i=0; i < PHY_REG_SIZE; ++i) {
		printf("reg %d: %d    ", i, registerBoard[i] );
  }
	printf("\n");*/
	...
	//	reorderBufferEntry *reorderBufferTable;
	//		malloc(reorderBufferSize * sizeof(reorderBufferEntry));
  ...
  //HW3_branch variables
	...
	int historyRegister = 0b0000000000000000;
	int printTrace = 0;
  ...
  while (true) {
		...
		//1. Commit
			for (i=0; i<wayNumber; i++)	{
				...
				//printf("%d",reorderBufferTable.front().doneCycle);

				if (reorderBufferTable.front().doneCycle != -1 &&
					reorderBufferTable.front().doneCycle <= currentCycle) {
					//printf("%d", reorderBufferTable.size());
					//insn_way_count=0;
					//freelist.insert(freelist.end(), phys_reg_to_free, phys_reg_to_free+4);
		...
		for (j = 0; j < (int)reorderBufferTable.size(); ++j) {
			...
			if (!reorderBufferTable.at(j).isIssued  
				...
				if (reorderBufferTable.at(j).isLoad) {
					...
					indexAddr = (reorderBufferTable.at(j).addressForMemoryOp / blockSize) % setSize;
					...
					//print cache
					blockAddress = reorderBufferTable.at(j).addressForMemoryOp / blockSize * blockSize;	//block offset bits set to zero
					//update cache
					if(missOrHit == 'h') {
							latency = 4;
		...
		for (i=0; i<wayNumber; i++) {
			...
			//printf("~~~~~~~~~~~~~~");
			//printf("\n microop: %d fetch cycle:  %d \n", microop.microopNumber, microop.fetchCycle);
		...
		if (result == EOF && reorderBufferTable.empty()) {
				printf("\nresult: %d %d", totalMicroops, currentCycle);
				break;
		}

  } //first while loop for cycles end
} //func_simulate end
			...
			for(i = 0; i < (*reorderBufferTable).size(); ++i) {
				...
				if ((*reorderBufferTable).at(i).isStore && 
				  ...
				  !((*reorderBufferTable).at(i).isIssued && ((*reorderBufferTable).at(i).doneCycle <= currentCycle))) {
					...
					if((*reorderBufferTable).at(i).addressForMemoryOp == (*microopPointer).addressForMemoryOp) {
						isReady = false;
					}
				}
			}
